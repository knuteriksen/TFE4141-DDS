...
begin
	...
	process (output_signal) begin
		...
		if (output_signal = '1') then
			result      <= mux_X_out;
			valid_out_i <= '1'; -- Data på output er gyldig valid_out(output) når data på inngangen er gyldig valid_in(input)
...
/*	process(clk, reset_n) begin
	   if (reset_n = '0') then
	       ...
	       if(valid_in = '1' and ready_in = '1') then
	           msg_last_2 <= msgin_last;
    	    end if;
    	    msg_last <= msg_last_2;
        end if;
     end process;*/
  
--  msgout_last <= msg_last;   	
-- ready_in <= ready_out;   -- Mottakersiden er klare(input ready_out), derfor er vi klare for å motta ny melding(output ready_in);
